...
export class HttpClientError extends HttpResponse {
    ...
    static getErrorInstance(statusCode: number) {
        const values = Object.values(this) as HttpClientError[];
        return values.find(
            err => err instanceof HttpClientError && err.statusCode === statusCode
        ) || null;
    }
...
export class HttpServerError extends HttpResponse {
    ...
    static getErrorInstance(statusCode: number) {
        const values = Object.values(this) as HttpServerError[];
        return values.find(err => err instanceof HttpServerError && err.statusCode === statusCode) || null;
    }
}
